...
1. **Event-first store port.** One `Store` port whose only mutation path is
   `journal.commit`, with read-only readers, a `DoltStore` adapter, and a
   `MemoryStore` fake, both passing one conformance suite. Writes that have no
   event today go through `commit` as declared, shrink-only unjournaled
   mutations. See `02-data-layer.md` §2.
...
- **R2.** No `Record<string, unknown>` event writes remain. Every write, in the
  runtime and in the MCP server, goes through `journal.commit`. Every write that
  isn't an event is listed in `store/unjournaled.ts`.
