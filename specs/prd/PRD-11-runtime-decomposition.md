...
5. Deduplicate kernel helpers into `kernel/` with one tested implementation
   each.
6. Make the server a single composition root, with RPC handlers in one module
   per namespace.
...
- **R1.** The `arch.imports` baseline reaches 0. After that there are no dynamic
  local `import()` calls, no cycles, and no deep imports.
- **R2.** No runtime module exceeds 1,000 LOC, and no function exceeds 200
  ...
  - Measured by the size report, and listed in the PR when exceeded with a
    reason.
- **R3.** Each directory has a `mod.ts` header stating its responsibility and
...
- Median lines an agent must read to change one engine stage: < 800, down from >
  4,000.
- Zero cycles and zero upward imports, enforced by the gate.
