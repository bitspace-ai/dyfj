...
6. Run one Workbench turn. Tool calls are not wired into the current turn loop yet.
...
- Session events are written to Dolt: `session_start`, `model_selected`, `model_response` or `error`, `tool_call` where applicable, `session_end`, and `budget_summary`. The current no-tool Workbench turn is verified by `deno task verify-workbench-events`.
...
1. [done] Add `prototype/src/workbench.ts` as the new Workbench entrypoint.
2. [done] Retire the legacy `prototype/src/index.ts` demo once the Workbench path owns `deno task start`.
