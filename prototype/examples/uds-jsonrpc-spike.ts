// transport-seam spike: duplex JSON-RPC 2.0 over a Unix domain socket.
//
// Proves the local transport for the workbench seam, including the
// hard part for the approval keystone: a SERVER-INITIATED request mid-turn (tool approval)
...
} catch { /* socket absent — fine */ }

const enc = new TextEncoder();
const dec = new TextDecoder();
const send = (c: Deno.Conn, m: unknown) =>
  c.write(enc.encode(JSON.stringify(m) + "\n"));
  ...
  for await (const msg of readMessages(conn)) {
    if (msg.method === "turn") {
      await send(conn, {
        jsonrpc: "2.0",
        method: "stream",
        params: { delta: "planning the edit…" },
      });
      await send(conn, {
        jsonrpc: "2.0",
        method: "stream",
        params: { delta: "needs a shell command" },
      });
      // server -> client REQUEST mid-turn (the approval round-trip)
      await send(conn, {
        jsonrpc: "2.0",
        id: "appr-1",
        method: "approval",
        params: { tool: "bash", command: "rm -rf build/" },
      });
...
await send(conn, {
  ...
  id: 1,
  method: "turn",
  params: { prompt: "tidy the build dir" },
});
for await (const msg of readMessages(conn)) {
  if (msg.method === "stream") {
    ...
    log.push(
      `approval REQUEST (server→client): ${msg.params.tool} · ${msg.params.command}`,
    );
    await send(conn, {
      jsonrpc: "2.0",
      id: msg.id,
...
console.log("=== transport-seam spike: duplex JSON-RPC 2.0 over UDS ===");
for (const l of log) console.log("  " + l);
const ok = log.some((l) => l.startsWith("approval RESPONSE")) &&
  log.some((l) => l.startsWith("final turn result"));
console.log(
  ok
