...
- Large JSON-RPC responses no longer truncate on the wire (BIT-230). `Deno.Conn.write` may write fewer bytes than requested, so a response larger than a single socket write was cut short and the peer's frame decoder waited forever for the missing newline (surfaced as a `sessions/list` hang). The peer now loops writes until the whole frame is flushed, with a 300 KB regression test.
- A slow JSON-RPC handler no longer blocks other requests on the same connection (BIT-230). The read loop awaited each handler before reading the next frame, so a long-running request wedged everything behind it (including its own cancel); handlers now dispatch concurrently while writes stay serialized, with a regression test.
