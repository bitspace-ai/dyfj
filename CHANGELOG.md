...
- **Bounded Test Runtime Supervision**: Prototype Vitest runs through `run-vitest.ts` now take an exclusive run lock, a wall-clock bound (`DYFJ_TEST_BOUND_SEC`, default 10 minutes or 3 minutes for a focused file/name), and a detached sibling reaper. Force-killing the runner reaps launcher/runtime leftovers, test sockets, and `start-test-runtime-*.lock` files. A second run refuses to start while a prior run is still alive. A hung suite fails the bound instead of occupying a worker indefinitely.
