...
`--runner fixture` selects the experimental external-agent path instead of the native model loop. Workbench launches the declared local fixture directly over ACP v1 stdio with a cleared, profile-selected environment and the resolved workspace, bridges permission requests through the existing approval channel (refusing permission when no handler exists), bounds protocol and process shutdown phases, and records runner-specific events and a runner receipt. Transport (`local_stdio`) remains distinct from the selected access route (`local_sidecar`) and its cost basis (`local_free`). The fixture is deterministic protocol coverage; it is not evidence that any vendor agent, subscription route, or agent-internal accounting is supported.
