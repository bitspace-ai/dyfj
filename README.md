...
- 2026-08-20 - Sequential ACP turns that share a Workbench session, workspace, and execution profile reuse one live worker and ACP session. Concurrent same-session work fails as busy. Turn cancellation keeps a healthy handle; protocol or process failure replaces it. Idle sessions retire on a TTL and capacity fails closed without eviction. UDS and SIGINT shutdown wait for in-flight creation, close resident handles, and surface a close failure. Standalone HTTP has no close hook; idle TTL and process exit retire those handles.
